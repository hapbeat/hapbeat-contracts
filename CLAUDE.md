...
- 仕様書（Event ID、Pack、Bridge API、メッセージフォーマット等）
...
- **Pack manifest schema** -- Pack ファイルの構造を変えると pack-tools および全 SDK に影響する
...
- Pack manifest へのフィールド追加・削除・型変更
...
2. **Pack manifest schema 定義** -- Pack ファイルのマニフェスト JSON Schema を策定する
...
```
Event ID → Pack → Bridge API → Message format → Versioning
```
...
- ローカルに配置された Pack ファイルと Event ID による触覚再生が、ネットワーク接続なしで完結する設計にする
...
- **Pack** -- 触覚パターンをまとめたファイル群。manifest schema はここで定義する
