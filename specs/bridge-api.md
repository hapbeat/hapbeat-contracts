...
1. SDK/アプリからブロードキャストアドレス（既定は `255.255.255.255:7700`）に PING パケットを送信
2. LAN 上の Hapbeat デバイスが PONG で応答

#### 送信元のブロードキャスト宛先（マルチホーム対応）

送信側は **limited broadcast（`255.255.255.255`）に加えて、ローカルの各サブネット宛て
ブロードキャスト（subnet-directed broadcast、例 `192.168.0.255`）へ PING を送ってよい**。
wire format は変わらない — 宛先アドレスの選び方の話であり、デバイス側の実装には影響しない。

**理由**: `255.255.255.255` はマルチホームホストでは**インターフェイスメトリックが最小の
1 本からしか送出されない**。Hyper-V / WSL2 / Docker が作る仮想アダプタは LAN ケーブルが
無くても常時 Connected で、Windows の既定で Wi-Fi より優先されることがある。この場合
探索パケットは Hapbeat のいるネットワークに一生届かない。subnet-directed broadcast は
直接接続サブネットの経路で解決されるためメトリックの影響を受けない。

送信側が守るべき規約:

- **宛先は各インターフェイスの実ネットマスクから `(ip & mask) | ~mask` で算出する。**
  `.255` 決め打ちは不可（/16 は `x.y.255.255`、/25 は `x.y.z.127`）。
  ネットマスクを取得できないプラットフォームは limited broadcast のみで動作してよい。
- **`255.255.255.255` は catch-all として常に候補に残す。** SoftAP 構成や単一 NIC 環境の
  互換性のため。
- **複数宛先へ送ってよいのは PING と CONNECT_STATUS のみ。** どちらも冪等なので二重着信は
  無害。**PLAY / STOP / STOP_ALL / STREAM_\* は必ず単一宛先へ送ること** — ファームウェア
  v0.3.0 未満は PLAY/STOP の seq 重複排除（§7.1）を持たないため、複数経路へ送ると触覚が
  2 回発火する。
- PONG が返った時点でそのデバイスのサブネットに確定し、以後のブロードキャストはそこへ
  向けることが望ましい。

なお mDNS で発見済みのデバイスへのユニキャストは、同じ理由でメトリックの影響を受けない
（hapbeat-helper がマルチホーム環境でも動作していたのはこのため）。
