...
- Pack のロードや wav 再生（デバイスファームウェアの責務）
  ...
  "devices": [
    {
      ...
      "installed_packs": ["com.example.game.explosion", "com.example.game.footstep"]
    }
  ]
}
```
...
### 5.4 `GET /api/v1/devices/{id}/packs`

デバイスにインストール済みの Pack 一覧を取得する。
...
```json
{
  ...
  "packs": [
    {
      "pack_id": "com.example.game.explosion",
      "version": "1.0.0"
    },
    {
      "pack_id": "com.example.game.footstep",
      "version": "1.2.0"
    }
  ]
}
...
### 5.5 `POST /api/v1/devices/{id}/packs/install`

デバイスに Pack のインストールを指示する。
...
| installed_packs | string[] | インストール済み Pack ID の一覧 |
...
| 409 Conflict | `1003` | 操作が競合している（例: Pack インストール中） |
