...
- **install-clips/**（`events` (command) がある場合は必須）: デバイスに **焼き込む** wav の格納先。Kit インストール時にデバイス側ストレージに転送され、command 再生時にデバイスがオンボードでデコード・再生する。サブディレクトリ配下に置くことも許容する。
- **stream-clips/**（`stream_events` がある場合は必須）: SDK 側 (Unity 等) が **ホストから UDP でストリーム送信** する wav の格納先。デバイス側には焼き込まれない。
...
| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `schema_version` | string | はい | manifest スキーマのバージョン（例: `"2.0.0"`） |
...
| `events` | object | はい | デバイスが flash 焼き込み clip を再生する event の辞書 (= 旧 schema 1.x の `events` を command-mode 専用に narrow 化したもの)。device kit_loader はこれを読み取る |
| `stream_events` | object | いいえ | SDK が UDP audio stream で送信する event の辞書（device は eventId を認識しない） |
| `install_clips` | object | いいえ | `install-clips/` 配下の WAV メタデータ辞書（`events` の参照ファイル） |
...
### events オブジェクト (command mode)

`events` は Event ID 文字列をキー、clip 定義オブジェクトを値とする辞書である。
**device は manifest 読み込み時にこのキー (eventId) をハッシュ化して event table を構築し、
SDK が送信する PLAY/STOP packet の `event_id` フィールドにこれらのキーが乗ってくる。**
すなわち `events` のキーは wire 上の eventId と一致する。

> **命名注**: schema 1.x まで `events` は command / stream_clip 等を混在させた単一辞書だったが、
> schema 2.0.0 (DEC-031) で `stream_events` を別 bucket に分離したため、`events` は実質
> command-mode 専用となった。`command_events` という symmetric な名前ではなく従来の `events`
> を維持しているのは、device firmware の kit_loader が既に `doc["events"]` を参照しており、
> 名前を保つことで firmware に変更を要しないため (将来別件で firmware を更新する機会に
> rename を検討)。

**キー**: Event ID 文字列（Event ID 仕様に準拠）

Event ID の形式（正準仕様は `specs/event-id.md`。以下は要約）:
- 正準形式: `<kit-name>.<file-name>`（例: `sample-kit.sine_100hz`）
- `<kit-name>` = この Kit の `name`（§4。`^[a-z][a-z0-9-]*$`）、`<file-name>` = clip ファイル名から `.wav` を除いた basename（`^[a-z][a-z0-9_-]*$`、英字始まり）
- 区切りは `.`（ドット）1 個。正規表現: `^[a-z][a-z0-9-]*\.[a-z][a-z0-9_-]*$`
- Studio が clip から自動合成する（`<kit-name>` が名前空間を兼ねるため Kit 間で ID が衝突しない）

**値（clip 定義オブジェクト）**:

| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `clip` | string | はい | `install-clips/` 配下の相対ファイル名（例: `"hit_medium.wav"`、サブディレクトリ可）。bare filename は `install-clips/<clip>` として解決される |
...
### stream_events オブジェクト

`stream_events` は SDK がホストから UDP audio stream で送信する event の辞書である。
**device はこのキーを認識しない。** STREAM_BEGIN/DATA/END packet には eventId フィールドが
含まれず、これらのキーは SDK 内部で AudioClip / binding と紐付けるラベルとして使われる。

**キー**: Event ID 文字列（`events` と同じ形式）。同一の Event ID が `events`
と `stream_events` 双方に存在することは **valid** であり、authored event を「FIRE と CLIP の両方で再生可能」
として表現する用途で使われる。
...
| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `clip` | string | はい | `stream-clips/` 配下の相対ファイル名（例: `"rain_loop.wav"`、サブディレクトリ可）。bare filename は `stream-clips/<clip>` として解決される |
| `description` | string | いいえ | このイベントの説明 |
| `tags` | string[] | いいえ | 分類用タグの配列 |
| `parameters` | object | いいえ | 再生パラメータ |
...
`stream_events.parameters` には `device_wiper` を含めない（SDK 送信側パラメータには関係しないため）。

### 同一 Event ID が両 bucket に存在する場合

`events.<id>` と `stream_events.<id>` が同じキーを持つ場合、両者の `parameters` は
**独立に読まれる**。読む側はその時点で使う bucket の値のみを参照する。Studio の「BOTH モード」
で authored された event はこのパターンで manifest に出力される。
...
`install_clips` は **install-clips/ 配下の WAV ファイル** のメタデータ辞書。キーは `events.<id>.clip` と同じ表記（bare filename、サブディレクトリを含む場合はそのまま）。値はメタデータオブジェクト。`stream-clips/` 配下のファイルはここには含めない。バリデーションや表示用途に使用する。
...
```json
{
  ...
  "events": {
    "basic-impacts.hit_medium": {
      "clip": "hit_medium.wav",
      "description": "中程度の打撃 (device baked)",
      "tags": ["impact", "basic"],
      "parameters": {
        "intensity": 0.8,
        "device_wiper": 64,
    ...
    "basic-impacts.hit_soft": {
      "clip": "hit_soft.wav",
      "description": "軽い打撃"
    },
    "basic-impacts.hit_hard": {
      "clip": "hit_hard.wav",
      "description": "強い打撃 (FIRE と CLIP 双方を持つ BOTH 例)",
      "parameters": {
        "intensity": 1.0,
        "device_wiper": 64
      }
  ...
  "stream_events": {
    "basic-impacts.hit_hard": {
      "clip": "hit_hard.wav",
      "description": "強い打撃 (SDK streaming variant — events と同 ID で BOTH 表現)",
      "parameters": {
        "intensity": 1.0
      }
    },
    "basic-impacts.rain_loop": {
      "clip": "rain_loop.wav",
...
- `events.<id>.clip` で指定する WAV は `install-clips/` 配下、`stream_events.<id>.clip` で指定する WAV は `stream-clips/` 配下に配置しなければならない。
- これら 2 つのディレクトリの外部にあるファイルを `clip` フィールドで参照することはできない。
- 相対パスで `../` を使用してディレクトリ外を参照することは禁止する。
- `clip` フィールドの値は bare filename（例: `"hit.wav"`）またはサブディレクトリ付き相対パス（例: `"impacts/hit.wav"`）。bucket に応じた subdir prefix (`install-clips/` / `stream-clips/`) を JSON 上に書く必要はなく、読む側が bucket から自動解決する。
