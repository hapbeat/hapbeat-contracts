...
**topic root**: トピックの第 1 階層（既定 `hapbeat`）は **クライアント側設定**（`set_broker_host` の
`topic_root`、NVS `mq_root`）で変更できる。同一 broker 上で複数システムを分離したい場合に使う。
以下の表は既定値で表記する。
...
- `transport=mqtt` の receiver は broker に接続し `<root>/play` / `<root>/stop` を subscribe する。
- 受信 payload の `target` を自アドレスと照合（`device-addressing.md`）し、一致時のみ `event_id` を再生。
- payload に `oled` があれば再生と同時に OLED へ ~2.5 秒間表示する（§4.1, item 9）。
...
- **ON（既定）**: 受信した `play` を **本体のいずれかのボタンが押されるまでループ再生**する。
  ボタン押下は STOP_ALL として消費され、その押下に割り当てられた通常動作（音量等）は発火しない。
- **OFF**: 1 回だけ再生（単発）。
