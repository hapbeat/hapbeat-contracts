...
| 6 | HP buffer_ms override（AS-BUILT, DEC-046） | ×10ms（4..50 = 40..500ms、実効上限 ~138ms） | mode 9（SOLID48）専用のジッタバッファ深さ override。**送信側**: TX の serial コマンド `set_stream_hp_buffer` で値を設定 + fleetTick 0xAC ビーコンに param 6 として同報。**受信側**: 0xAC param 6 受信で `audioStreamSetBufferMs()` を適用（mode 9 のときのみ、他 mode は無視）。Studio → TX(Web Serial) → 0xAC → 艦隊一括で HP レイテンシ調整可。既定 120ms | runtime |
