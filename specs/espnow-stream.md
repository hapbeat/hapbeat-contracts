...
| 9 | **SOLID48**（AS-BUILT v1, DEC-046） | Opus 48k **stereo** (cplx**1**, RESTRICTED_LOWDELAY, 64kbps CBR) | 10ms | ×2（隣接pb のみ） | HP 既定 120ms（Studio の `set_stream_buffer` 明示設定があればそちらを優先、実効上限 ~138ms） | ビルド済・**実機未検証**（G-A1〜G-A4 未実施） |
