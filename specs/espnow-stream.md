...
| 9 | **SOLID48**（AS-BUILT v1, DEC-046） | Opus 48k **stereo** (cplx**1**, RESTRICTED_LOWDELAY, 64kbps CBR) | 10ms | ×2（隣接pb のみ） | HP 固定 120ms（実効上限 ~138ms） | ビルド済・**実機未検証**（G-A1〜G-A4 未実施） |

> **AS-BUILT 注記（mode 9 SOLID48, DEC-046, 2026-07-14 更新）**: mode 9 は**実装済み（両側ビルド SUCCESS）・実機未検証**。数百 ms の遅延を許容し通信頑健性を最優先する会場向けプロファイル（`docs/instructions-v4-solid-audio-202607112200.md` が原設計）。**v1 の実装は原設計から2点簡略化した**（下記）:
> 1. **遅延リピート（T-12 時間分散冗長）は不採用**。実装調査で受信機の seq 回復が**前方専用 SPSC リング**と判明し、12 パケット遅れて届くフレームは「過ぎたスロット」に充填できず drop/resync するため、原設計の「既存 seq 回復で自然充填」は成立しない（reorder バッファ再設計が別途必要＝将来課題）。v1 は **隣接 piggyback ×2（pb2）+ 深い HP バッファ**で頑健性を確保する。
> 2. **受信の 48k decode / HP / 触覚 fan-out は自前実装せず、Wi-Fi/UDP の format=2 経路（`message-format.md` §0x31, d214 で実績）に委譲**。専用ビルド env `duowl_v4_stream_espnow_hp48`（`-DESPNOW_HP48`）で `DUOWL_V4_DUAL_CODEC` を有効化し、mode 9 パケットの Opus フレームを length-prefix で `audioStreamFeedData(format=2)` に渡す。HP ring は現行 8192f（~138ms cap）を流用（PSRAM 415ms 深リングは将来課題）。既存 env `duowl_v4_stream_espnow`（16k, mode 0-8）と v3 艦隊は**完全に不変**（新 env で分離）。
>
...
> - **HP バッファは固定 120ms**（実効上限 ~138ms＝現行 8192-frame HP ring）。数百 ms の「サイレントフェス」耐性には PSRAM 深リングが必要（将来課題）。
> - **§3.5 param 6（mode-9 HP buffer override）は spec のみ・v1 未配線**（送信側に param-6 経路が無いため受信側も未実装＝デッドコード回避）。両側同時配線を将来対応。
> - v3/旧受信機への影響なし（未知 mode_id は範囲チェックで無視、plain env は mode 9 を 16k stereo 触覚として graceful degrade）。**hp48 env では mode 0-8 の HP は無音**（16k 内容は 48k HP ring に載らない設計・触覚は正常）。
>
...
#### mode 9（SOLID48）body — AS-BUILT v1（primary のみ実装）+ 将来設計（遅延リピート）

> **v1 実装 = 下記 (a) primary パケットのみ**（既存 Opus mode と同一 wire、pb2）。**(b) 遅延リピートは未実装（将来課題）** — 受信機が前方専用リングのため現状では効かない（上記 AS-BUILT 注記参照）。(b) を有効化するには受信側に seq-indexed reorder バッファを追加する必要がある。

数百 ms のバッファ許容を前提に、**バースト全滅への耐性を「時間分散した exact copy」で稼ぐ**プロファイル。将来のフル版は 1 フレーム(10ms)ごとに **2 種類のパケット**を送る:

**(a) primary パケット** — mode 1-5/8 と同じ Opus body 慣習（§3.3 準拠）+ pb=1（直前フレームの隣接 piggyback、単発ロス即回復用）:
...
**(b) 遅延リピートパケット** — 毎フレーム追加送信。**T-12（≈120ms 前）のフレームを、独立した mode_id=9 パケットとして単独再送**する。pb は付けない（このパケット自体が「別フレームの遅延コピー」であり、既存の seq 一致リカバリで自然に充填される。**新規 wire フィールド不要**）:

| オフセット | フィールド | 型 | 説明 |
|---|---|---|---|
| 0-3 | 共通ヘッダ | — | type=0xAA / mode_id=9+RELAYED / **seq = T-12 時点の seq**（現在の seq ではない） / pb_count(=0) |
| 4 | frame_len | uint16 LE | T-12 フレームの長さ |
| 6 | frame_data | uint8[frame_len] | T-12 フレーム本体（再送データそのまま、re-encode しない） |

- 受信機は primary と遅延リピートを**同じ seq 空間・同じ受信処理**（欠番検出→充填）で扱う。primary で欠落した seq が後から遅延リピートで埋まれば、その時点で ring の該当スロットへ格納する（後付けスプライスではなく seq 順の ring 供給）。
- **≤120ms のバースト全滅**（primary + 隣接pb の両方が同一バーストで消える最悪ケース）を、120ms 遅れて届く独立コピーで回復する。G-A3 で不足が判明した場合、**T-12 + T-25 の 2 段時間分散**へ拡張可能（wire 構造は変わらず、追加の遅延リピートパケットをもう 1 種類送るだけ）。
- **airtime 目安（6M, 96kbps）**: primary(pb1込み) ~383µs + 遅延リピート ~220µs ≈ 1 ソースあたり **~6%**（実測は G-A3）。
- **受信機バッファ**: prebuf/target **300ms**・max **450ms**（HP ring を PSRAM 化して ~20000f=415ms に拡張、既存 8192f=170ms から増量。Opus decoder state は internal RAM 維持 — PSRAM 配置は decode 速度 2 倍化の副作用が既知のため避ける）。
- **RESYNC_GAP**: mode 9 選択時は受信機側で **64** へ拡大（既定 20 のままだと遅延リピートの seq 差 12 を「別ストリーム接合」と誤認し resync してしまう。§7.1.1 の一般原則の mode 9 向け override）。`OPUS_RECOVER_CAP`（PLC 連発の上限）も同窓へ拡大。
- 実装確定は G-A1（送信側 48kHz stereo encode 実時間ベンチ、CoreS3）通過後。NG（duty ≥60%）の場合は 24kHz stereo への縮退を検討（wire の frame_len は可変長のため fallback してもフィールド構造は変わらない）。
...
### 3.5 fleet-tune ビーコン（type = 0xAC, DEC-043 P5 / DEC-045 epoch 同期）

会場 60 台規模の全受信機へ**電波経由でランタイム調整値**を配る **4 または 5 バイト**の可変長ビーコン。**runtime のみ・NVS 非保存**（re-boot で mode 既定へ復帰 = 置き忘れ防止。relay-test と同思想。ただし param 5 は例外＝下表参照）。
...
| 4 | epoch（任意） | uint8 | DEC-045。**送信機は必ず付与する (MUST)**（5 バイト送信）。受信側は無くても動作する（4 バイト＝互換モード）。単調増加カウンタ（wrap 対応）。同一 param の複数送信元（TX 間）で「どちらが新しい変更か」を決めるため。 |

| param_id | 意味 | value | 適用先 | 保存 |
|---|---|---|---|---|
| 1 | buffer_ms override | 0=mode 既定 / 4..60 | ジッタバッファ深さ | runtime |
| 2 | selection enable | 0/1 | 配信レートベース選択（§7.1.2）の有効/無効 | runtime |
...
| 6 | buffer_ms override（**spec のみ・v1 未配線**, DEC-046） | ×10ms（10..50 = 100..500ms） | mode 9（SOLID48）専用のジッタバッファ深さ override。**v1 では送受とも未実装**（送信側に param-6 経路が無く、受信側だけ足すとデッドコード + 将来 OOB 種になるため）。mode 9 は固定 120ms（実効上限 ~138ms）。将来 PSRAM 深リング化とあわせて両側同時配線する | runtime |
...
- リピータは 0xAA と同じ bit7 ルールで 0xAC も中継する（§7.2、**4〜8 バイトの範囲で**中継しなければならない (MUST) — 5 バイト epoch 付きビーコンを取りこぼすと、そのリピータ経由の受信機だけ fleet-tune 全般が届かなくなる）。RELAYED=1 は中継しない。
- **保存ポリシー**: param 1..4 は runtime のみ・NVS 非保存（再起動で mode 既定へ復帰＝置き忘れ防止）。**param 5（volume_max）は例外で NVS 保存する (MUST)** — 会場ごとの「音量レンジ」設定であり、受信機のバッテリ交換・再起動をまたいで維持されるべきため。送信機が param 5 を配信していない間は、受信機の NVS 値（既定 100=フルスケール）とローカル操作（受信機 CONFIG モードの Btn2 で 6 段循環）が支配する。送信機が param 5 を配信中は 5s 再送でそれが権威（ローカル Btn2 は次のビーコンで上書きされ得る）。
- **volume_max の適用は I2C/codec + NVS を伴う**ため、受信機は recv コールバック（Wi-Fi タスク）内で適用してはならない (MUST NOT)。値だけを保持し、UI/loop タスクが取り込んで適用・保存する（volume ハードの単一書き手を保つ）。

**epoch 同期（DEC-045、2026-07-12 改定 — strictly-older 拒否・同値受理）**:
- **受信機**: param 毎に最後に適用した epoch を保持。epoch 付き（5 バイト）ビーコンは、**厳密に古い epoch（wrap-safe に「過去」と判定できる時）だけ拒否しなければならない (MUST)**（`(int8_t)(epoch - last_epoch) < 0` → 拒否。それ以外＝初回受信・同値・新しい epoch は適用対象）。epoch 無し（4 バイト、旧仕様互換）は**常に適用**（互換モード）。
...
- **mode 別 override（DEC-046・将来）**: mode 9 のフル版（遅延リピート採用時）は **T-12（seq 差 12）の遅延リピートパケット**を正規経路に使うため `RESYNC_GAP=64` への拡大が要る。**v1 は遅延リピート未採用のため override せず既定 20 のまま**（§3.4 mode 9 AS-BUILT 注記参照）。遅延リピートを実装する際にセットで導入する。
...
- 受信機は §3.5 の 0xAC を**ロック中ソースからのみ**適用（param 1..4 は runtime・NVS 非保存 / **param 5 volume_max は NVS 保存**）。リピータは 0xAA と同じ bit7 ルールで 0xAC を中継（RELAYED=1 は中継しない）。
- **新 param_id の追加のみ受信機 reflash が要る**。既存 param の新値・LONGRANGE 追加・諸元調整はすべて**送信機側の変更だけ**で受け止まる（受信機の「事前 1 回 flash」の設計目標）。
- **volume_max（param 5）**: 送信機 CoreS3 の **HOME > RX VOL** から 6 段（10/20/40/60/80/100%）を選ぶ → 全受信機が上限を再マップ（0..ceiling、下限 0=mute 固定）して分解能を上げ、NVS 保存。会場信号レベルに合わせて上限を絞るほどノブ全域が有効レンジに集中する。受信機ローカルの最終手段として CONFIG モードの **Btn2** で同 6 段を循環（送信機が配信中はビーコンが優先）。
