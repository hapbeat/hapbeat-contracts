...
**epoch 同期（DEC-045）**:
- **受信機**: param 毎に最後に適用した epoch を保持。epoch 付き（5 バイト）ビーコンは、**初回受信、または wrap-safe に「新しい」と判定できる時だけ**適用しなければならない (MUST)（`(int8_t)(epoch - last_epoch) > 0`）。epoch 無し（4 バイト、旧仕様互換）は**常に適用**（互換モード）。目的: ある送信機の 0xAC を受信機がロック中ソース経由で一度適用した後、**別の（値が古い）送信機**に handoff した際、その送信機からの遅れて届いた/重複した同一 param のビーコンで**新しい値が古い値へ巻き戻されない**ようにするため。
