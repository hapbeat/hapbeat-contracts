...
### 3.4 mode_id とモード別 body（AS-BUILT, 2026-07-08／mode 6・mode 7 は 2026-07-09 追加）
...
|---|---|---|---|---|---|---|
| 0 | **RAW** | IMA-ADPCM 16k **stereo** | 4ms | ×1 | 480f=30ms | 36ms |
| 1 | **FAST** | Opus 8k mono (cplx2) | 5ms | ×3 | 256f=16ms | 38ms |
...
> **AS-BUILT (2026-07-09)**: pb は per-mode の piggyback 深さ（送受一致必須。受信は wire `pb_count` を最大 3 までパース）。RX buffer は `RX_MODE[mode].buf_frames`（モード切替で適用）。FAST/LITE は当初 128f=8ms だったが RF バースト(maxgap≈4)で under-run したため 256f=16ms に深化。実測 E2E 遅延は tx アナログ入力→rx アナログ出力の包絡クロス相関（≈ frame蓄積 + RX buffer + codec分。codec分は ADPCM ~2-4ms / Opus ~15-17ms、差 ~13ms が LITE/TURBO の低遅延要因）。

- mode 0（RAW）の body は §3.1（ただし mode-aware ヘッダでは offset 1=mode_id, offset 2=seq, offset 3=pb_count に読み替え。§3.1 の offset 1=seq/offset 2=num_frames は mode-id 導入前の記述）。
- mode 1-5（Opus）の body は §3.3 参照。
