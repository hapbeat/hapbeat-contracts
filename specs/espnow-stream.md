...
| 9 | **SOLID48**（DRAFT, DEC-046） | Opus 48k **stereo** (cplx0-2, RESTRICTED_LOWDELAY) | 10ms | ×1（隣接pb） + 遅延リピート(T-12) | ~20000f=415ms（PSRAM） | 未実測（G-A1〜G-A4 未実施） |

> **DRAFT 注記（mode 9 SOLID48, DEC-046, 2026-07-12）**: mode 9 は**未実装・未実機検証**。数百 ms の遅延を許容し通信頑健性を最優先する会場向けプロファイル（`docs/instructions-v4-solid-audio-202607112200.md` が原設計）で、送信側の実時間 encode ベンチ（G-A1）を含む 4 ゲート（G-A1〜G-A4）を通過するまで「確定 wire」に昇格しない。下記 body 定義は既存 Opus mode（1-5, 8）の body 慣習を踏襲した**設計案**であり、実装セッションは G-A1 通過後に実コード（`audio_source.cpp` の `MODE_DEFS` / `espnow_stream.cpp` の `RX_MODE`）と突き合わせて確定させること。v3/旧受信機への影響なし（未知 mode_id は範囲チェックで無視、mode_id 0-8 の既存挙動は無変更）。
>
...
#### mode 9（SOLID48）body — DRAFT, DEC-046（未実機検証）

数百 ms のバッファ許容を前提に、**バースト全滅への耐性を「時間分散した exact copy」で稼ぐ**プロファイル。1 フレーム(10ms)ごとに **2 種類のパケット**を送る:
...
| 6 | buffer_ms override（拡張, DRAFT, DEC-046） | ×10ms（10..50 = 100..500ms） | mode 9（SOLID48）専用のジッタバッファ深さ override。param 1（4..60ms）ではレンジ不足のため新設。mode 9 以外では無視 | runtime |
...
- **mode 別 override（DRAFT, DEC-046）**: mode 9（SOLID48）は仕様上 **T-12（seq 差 12）の遅延リピートパケット**を正規の受信経路として使うため、既定 `RESYNC_GAP=20` のままだと遅延リピートを「別ストリーム接合」と誤認して再同期してしまう。mode 9 選択時は `RESYNC_GAP=64` へ拡大する（§3.4 mode 9 body 参照）。他 mode（0-8）は既定 20 のまま変更なし。
