...
SDK、Bridge、Transmitter、デバイスファームウェア、Pack など複数のコンポーネントが連携するため、
...
| Pack manifest schema バージョン | `schema_version` フィールド | manifest.json 内で宣言 |
...
| Pack バージョン | semver | 各 Pack の `version` フィールド |

## 4. 互換性マトリクス

### 4.1 Pack と Device Firmware

Pack の `target_device` セクションで互換性を管理する。

- `target_device.firmware_version_min`: この Pack が動作する最低ファームウェアバージョン
- `target_device.firmware_version_max`（省略可能）: この Pack が動作する最大ファームウェアバージョン

Bridge はデバイスのファームウェアバージョンと Pack の要件を照合し、
互換性がない場合はエラーを返す。
...
| Pack manifest schema の必須フィールド追加 | 既存の manifest が不正になる |
...
| 変更内容 | 理由 |
|---------|------|
| Pack manifest schema のオプショナルフィールド追加 | 既存の manifest は引き続き有効 |
