...
**Request:**
```json
{"cmd": "set_stream_buffer", "buffer_ms": 0, "persist": true}
```
...
- `persist` (bool, 既定 true): §4c の共通仕様（`false` = live のみ・NVS スキップ）。
...
読み出しは `get_info` の **`audio` object**（DuoWL v4 のみ）: `{"pam_db": 24, "lineout_db": 6, "boost_db": 0, "hp_db": -10, "input_mode": "output", "stream_buffer_ms": 0, "av_delay_ms": 0}`（`av_delay_ms` は **signed −100..+100**、`audio-dsp-config.md` §3）。

**`persist` フィールド（下記 set_* 全コマンド共通・任意・既定 `true`）**: `false` で「live 適用のみ・NVS 書込みスキップ」。Studio のスライダー等はドラッグ中に `persist:false` を debounce 送信し、離した時に `persist:true` で確定する運用（NVS 摩耗回避）。firmware 側にも debounce watchdog があり、`persist:false` の変更が ~1.5 秒確定されない場合は自動で NVS へ flush する（確定取りこぼし・切断対策）。live 効果（codec レジスタ / s_cfg / リング）は persist に関わらず即時適用される。
...
**Request:**
```json
{"cmd": "set_dac_boost", "boost_db": 0, "persist": true}
```
...
**Request:**
```json
{"cmd": "set_headphone_volume", "hp_db": -10, "persist": true}
```
...
- `persist` (bool, 既定 true): §4c の共通仕様（`false` = live のみ・NVS スキップ）。
- **本体ボタン（SW4/SW5）でも変更可**。ボタン変更時は `{"event":"volume_changed", …, "hp_db": <int>}` を非請求で push するので、Studio は HP 音量表示を追従できる。
- NVS 永続。
